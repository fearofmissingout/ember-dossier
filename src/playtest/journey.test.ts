...
describe("journey route generation", () => {
  ...
  test("repeated counters can break enemy posture", () => {
    ...
    if (first.combat) {
      ...
      first.combat.intentLabel = "Brace";
      first.combat.intentText = "Armor rises this round. Tactic breaks the posture.";
    }
    const second = resolveCombatRound(first, "tactic", squad, 60);
    if (second.combat) {
      second.combat.intent = "prowl";
      second.combat.intentLabel = "Prowl";
      second.combat.intentText = "It is looking for an opening. Strike or tactic can interrupt.";
    }
